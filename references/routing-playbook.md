...
- “帮我分析下某天堡垒机器的使用情况” -> 报告/使用分析
- “帮我分析下20260310的堡垒机使用情况” -> 报告/使用分析
- “帮我分析下3月10号的堡垒机器使用情况” -> 报告/使用分析
- “帮我分析下2026-03-10到2026-03-24的堡垒机使用情况” -> 报告/使用分析
- “帮我分析下上周的堡垒机使用情况” -> 报告/使用分析
- “看看某天使用情况” -> 报告/使用分析
...
- “想看上周谁登录最多” -> 报告/使用分析
- “近一个月示例用户登录多少次” -> 审计调查 / 页面同款登录日志
- “近一个月示例用户A和示例用户B各登录多少次” -> 审计调查 / 页面同款登录日志
- “查昨天 30 条登录日志” -> 审计调查
...
- `帮我分析下某天堡垒机器的使用情况`
- `帮我分析下20260310的堡垒机使用情况`
- `帮我分析下3月10号的堡垒机器使用情况`
- `帮我分析下2026-03-10到2026-03-24的堡垒机使用情况`
- `帮我分析下上周的堡垒机使用情况`
...
- `某某用户有哪些资产` / `某某用户在 Default 组织下有哪些资产`
- `某某用户有哪些节点`
- `某某用户在某资产下有哪些账号` / `某某用户在某资产下有哪些协议`
- `which assets can this user access` / `which nodes can this user access`
- 这类结果型问法先走 `jms_diagnose.py user-assets` / `user-nodes` / `user-asset-access`
...
- `登录日志` / `会话记录` / `命令记录` / `文件传输`
- `某个用户` / `多个指定用户` + `登录多少次` / `成功登录多少次` / `失败登录多少次`
...
- 没有时间范围的 `谁用得最多`
- 句子里带了“用户”二字，不代表一定是权限关系；结果型问法优先回到用户有效访问范围
...
2. 命名用户的“近一个月登录多少次 / 成功登录多少次 / 失败登录多少次”不走模板；即使带时间词，也优先页面同款登录日志查询
3. 除上述明确例外外，只要核心对象是某一天或某一段时间内的 JumpServer 使用数据分析 -> 必走模板报告
4. “审计分析”不是自由文本摘要的默认入口；如果带有明确日期/时间范围且关注整体使用情况，仍然走模板
...
- 没给组织且存在多个可访问组织：返回 `candidate_orgs`，并通过 `user_message` / `action_hint` 明确要求先选择查询组织。
- 当前组织已生效且仍有其他可访问组织：继续返回 `switchable_orgs`，并通过 `org_context_hint` 明确提示当前查询范围固定在哪个组织。
- 用户明确说“在 Default 组织下”“在某组织下”这类表达时，视为显式组织；优先在单次命令内带 `--org-id` / `--org-name` 执行 `user-assets` / `user-nodes` / `user-asset-access`，不要回退成“检查所有组织”或授权规则说明。
...
- 先明确说明“报告已生成”
- 回显报告文件路径
- 回显文件存在性与大小：`output_exists`、`output_size_bytes`、`output_size_human`
- 回显模板路径
- 回显字段元数据路径
- 回显生效组织（`effective_org`）
- 有其他可切换组织时回显可切换组织（`switchable_orgs`）
- 回显已查询命令存储 ID 列表（`queried_command_storage_ids`）
- 回显已查询命令存储数量（`queried_command_storage_count`）
- 回显时间范围
- 回显校验摘要（`validation_summary`）
- 给最终 HTML 报告
...
- 说明阻塞原因
- 给候选组织（`candidate_orgs`）或候选对象（`candidate_objects`）
- 组织未确定时补充 `reason_code`、`user_message`、`action_hint`、`candidate_org_count`
- 给下一步安全动作
...
- “后台已生成 HTML，但回复只给摘要” -> 错误；必须先回显报告产物信息
