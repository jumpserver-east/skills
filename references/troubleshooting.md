...
| `需要先选择组织后才能继续查询。` / `reason_code=organization_selection_required` | 未选组织，且当前环境不能自动代选组织 | 先看 `candidate_orgs` 和 `suggested_commands`，再执行 `python3 jumpserver-runtime-setup/scripts/jms_diagnose.py select-org --org-id <org-id> --confirm` |
...
| 误用 | 正确做法 |
|---|---|
| 为标准流程写临时 SDK Python 文件 | 回到正式的子 skill wrapper，例如 `jumpserver-object-query/scripts/jms_query.py`、`jumpserver-runtime-setup/scripts/jms_diagnose.py`、`jumpserver-usage-reporting/scripts/jms_report.py` |
